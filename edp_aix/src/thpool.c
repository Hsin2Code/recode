...
static int  thread_init_(thpool_* thpool_p, struct thread_** thread_p, int id);
static void* thread_do_(struct thread_* thread_p);
static void  thread_hold_();
static void  thread_destroy_(struct thread_* thread_p);
...
struct thpool_* thpool_init(int num_threads){
    ...
    thpool_p->threads = (struct thread_**)malloc(num_threads * sizeof(struct thread *));
    ...
    for (n=0; n<num_threads; n++){
        thread_init_(thpool_p, &thpool_p->threads[n], n);
        if (THPOOL_DEBUG)
...
void thpool_destroy(thpool_* thpool_p){
    ...
    for (n=0; n < threads_total; n++){
        thread_destroy_(thpool_p->threads[n]);
    }
...
static int thread_init_(thpool_* thpool_p, struct thread_** thread_p, int id){
    ...
    *thread_p = (struct thread_*)malloc(sizeof(struct thread_));
    if (thread_p == NULL){
        fprintf(stderr, "thread_init_(): Could not allocate memory for thread\n");
        return -1;
    }
    ...
    pthread_create(&(*thread_p)->pthread, NULL, (void *)thread_do_, (*thread_p));
...
static void* thread_do_(struct thread_* thread_p){
...
#else
    //fprintf(stderr, "thread_do_(): pthread_setname_np is not supported on this system");
#endif
    ...
    act.sa_flags = 0;
    act.sa_handler = thread_hold_;
    if (sigaction(SIGUSR1, &act, NULL) == -1) {
        fprintf(stderr, "thread_do_(): cannot handle SIGUSR1");
    }
...
/* Frees a thread  */
static void thread_destroy_(thread_* thread_p){
    free(thread_p);
}
